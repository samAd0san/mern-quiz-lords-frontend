...
import { FaUser, FaLock, FaArrowRight, FaGraduationCap, FaChalkboardTeacher } from "react-icons/fa";
...
function Login() {
  ...
  const onFacultyInputChange = (evt) => {
    const newCredentials = { ...facultyCredentials, [evt.target.name]: evt.target.value };
    setFacultyCredentials(newCredentials);
  };
  ...
  const onFacultyLogin = async (evt) => {
    evt.preventDefault();
    setLoading(true);
    setError(false);
    
    try {
      // Check if the credentials match the fixed faculty credentials
      if (facultyCredentials.id === "1357" && facultyCredentials.password === "1357") {
        // Set a flag in localStorage to indicate faculty login
        localStorage.setItem("isFaculty", "true");
        localStorage.setItem("facultyId", facultyCredentials.id);
        
        // Create a mock token for faculty
        const mockToken = btoa(`faculty:${facultyCredentials.id}`);
      ...
      <div className="max-w-4xl w-full bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-100">
        <div className="flex flex-col md:flex-row">
          ...
          <div className="md:w-1/2 bg-gradient-to-br from-primary to-secondary p-8 md:p-12 flex flex-col justify-center text-white">
            <div className="mb-8">
              ...
              <p className="text-white/80 text-lg leading-relaxed">
                {isFacultyLogin 
                  ? "Access the faculty dashboard to manage quizzes and view results."
            ...
            <div className="mt-auto">
              <div className="flex items-center space-x-4 mb-6">
                <div className="bg-white/20 p-3 rounded-full">
                  {isFacultyLogin ? <FaChalkboardTeacher size={20} /> : <FaUser size={20} />}
                </div>
                <div>
                  <h3 className="font-semibold">{isFacultyLogin ? "Faculty Access" : "Student Access"}</h3>
                  <p className="text-sm text-white/80">
                    {isFacultyLogin ? "Manage quizzes and view results" : "Take quizzes and track your results"}
                  </p>
                </div>
              </div>
          ...
          <div className="md:w-1/2 p-8 md:p-12">
            <div className="space-y-6">
              <div className="text-center space-y-2">
                <h2 className="text-3xl font-extrabold text-primary">
                  {isFacultyLogin ? "Faculty Login" : "New to Lords Quiz?"}
                </h2>
                <p className="text-lg text-gray-600">
                  {isFacultyLogin ? "Access the faculty dashboard" : "Create an account to get started"}
                </p>
              </div>
              ...
              <div className="relative my-8">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-4 bg-white text-gray-500">
                    {isFacultyLogin ? "Faculty Access" : "Or sign in to your account"}
                  </span>
                </div>
              </div>
            </div>
            ...
            {isFacultyLogin ? (
              <form className="mt-8 space-y-6" onSubmit={onFacultyLogin}>
                ...
                <div>
                  ...
                  >
                    <span className="absolute left-0 inset-y-0 flex items-center pl-3">
                      <FaChalkboardTeacher className="h-5 w-5 text-white/80 group-hover:text-white transition-colors duration-300" />
                    </span>
                    Faculty Login
                  </button>
                </div>
            ...
            <div className="mt-6 text-center">
              ...
              >
                {isFacultyLogin ? "Switch to Student Login" : "Switch to Faculty Login"}
              </button>
            </div>
          </div>
        </div>
